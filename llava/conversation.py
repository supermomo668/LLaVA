...
class Conversation:
    """
    A class that keeps all conversation history and memory of states.
    Broker request processing to the endpoint
    """
    system: str
    roles: List[str]
    ...
    def to_gradio_chatbot(self):
        """
        KEY processing function of Input Payload
        """
        ret = []  # some kind of stora
        for i, (role, msg) in enumerate(self.messages[self.offset:]):
            if i % 2 == 0:   # only appends input from Human(User)
                if type(msg) is tuple:
                    import base64
                    from io import BytesIO
                    # Expand input payloads
                    msg, image, image_process_mode = msg
                    max_hw, min_hw = max(image.size), min(image.size)
                    aspect_ratio = max_hw / min_hw
                    max_len, min_len = 800, 400
...
conv_ezout_v0 = Conversation(
    ...
    messages=(
        ...
        ("Assistant",
         ...
         "Don't forget to check out the fresh produce section for fruits and vegetables to add to your meals or enjoy as snacks. And of course, you can satisfy your sweet tooth with dessert options like ice cream, cookies, or fresh bakery items available at the store. With these ingredients, you'll have a wide range of meal possibilities to enjoy!"
        )
    ),
    offset=2,
    sep_style=SeparatorStyle.SINGLE,
...
default_conversation = conv_templates.get('default')
